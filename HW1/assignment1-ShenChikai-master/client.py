...
if len(sys.argv) == 1:
	...
	try:
		...
		print("Please enter the information in its correct format.")
		print("Please eneter information in format of 'server.py -s <serverAddr> -p <serverPort> -n <clientName>'. \nPlease include the whitespace.")
else:
	print("Please eneter information in format of 'server.py -s <serverAddr> -p <serverPort> -n <clientName>'. \nPlease include the whitespace.")
	exit()
...
try:
	...
	print("connected to server and registered", clientName)
	print("Server:", regRet)
	...
	# server not up
	closed = True
	print("server on this port has not been started!")
	print("terminating server...")
	sock.close()
